...
////
//// Compile/run: $> g++ -fopenmp -o xxx hw2_pro5_partA.cpp
...
int main(int argc, char* argv[])
{
 ...
 #pragma omp parallel num_threads(thread_count)// changed 
 {
 double localSum = Trap(a, b, n); //variable local to each thread 
 #pragma omp critical
 global_result += localSum; //  variable summed using critical section 
 }
